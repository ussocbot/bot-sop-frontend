...
const { fetchJson } = require("../feishu");
const { getSubmissionAccess } = require("../submission-access");
...
async function fetchFieldTypes(access, tableId) {
  const url = new URL(
    `/open-apis/bitable/v1/apps/${encodeURIComponent(access.appToken)}` +
  ...
  return new Map(
    (result.data?.items || []).map(field => [field.field_name, Number(field.type)])
  );
}

function simpleFieldText(value) {
  if (value === null || value === undefined) return "";
  if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") {
    return String(value);
  }
  if (typeof value === "object") {
    return String(value.text ?? value.name ?? value.label ?? value.value ?? value.id ?? "");
  }
  return "";
}

async function normalizeResourceFields(access, tableId, fields) {
  const fieldTypes = await fetchFieldTypes(access, tableId);
  if (!fieldTypes.has("Content Name")) {
    const error = new Error('The Documentation table must contain a primary field named "Content Name".');
    error.status = 500;
    throw error;
  }
  const normalized = {};
  ...
  for (const [name, value] of Object.entries(fields)) {
    // Optional submission fields are allowed to be absent from the Base. Do not
    // make the entire submission fail because a table does not use one of them.
    if (!fieldTypes.has(name)) continue;
    const type = fieldTypes.get(name);

    // Feishu type 1 is text, type 3 is single select, and type 4 is multi-select.
    // Normalize only those flexible values; person, date, checkbox, link, and
    // attachment fields must retain their structured values.
    if (type === 1 && Array.isArray(value)) {
      normalized[name] = value.map(simpleFieldText).filter(Boolean).join(", ");
    } else if (type === 3 && Array.isArray(value)) {
      normalized[name] = value.map(simpleFieldText).find(Boolean) || "";
    } else if (type === 4) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (kind === "resource") {
      ...
      const normalizedFields = await normalizeResourceFields(access, tableId, fields);
      const record = await createRecord(access, tableId, normalizedFields);
