...
const { fetchJson, getTenantToken, fetchTableRecords } = require("../feishu");
...
module.exports = async function sendToMe(req, res) {
  ...
  try {
    ...
    const messageUrl = `${apiOrigin}/open-apis/im/v1/messages?receive_id_type=open_id`;
    const messageHeaders = {
      Authorization: `Bearer ${tenantToken}`,
      "Content-Type": "application/json; charset=utf-8"
    };
    let response;
    let deliveryFormat = "interactive";

    try {
      response = await fetchJson(messageUrl, {
        method: "POST",
      ...
      if (cardError.feishuCode === 99991663 || cardError.status === 403) throw cardError;
      deliveryFormat = "text";
      const fallbackText = [
        title,
        summary ? `Summary\n${clipped(markdownValue(summary), 1000)}` : "",
        instruction ? `Guidance\n${clipped(markdownValue(instruction), 4000)}` : "",
        closingGuidance ? `Closing Guidance\n${clipped(markdownValue(closingGuidance), 1800)}` : "",
        ticketTagDisplay ? `Ticket Tags\n${clipped(markdownValue(ticketTagDisplay), 800)}` : "",
        relatedLinks.length ? `Related Resources & Tasks\n${relatedLinks.slice(0, 20).map(item => `${item.title}: ${item.url}`).join("\n")}` : "",
        resourceUrl ? `Resource: ${resourceUrl}` : "",
        websiteUrl ? `Open in BOT SOP: ${websiteUrl}` : ""
      ].filter(Boolean).join("\n\n");
    ...
    return res.status(502).json({
      error: "Unable to send this entry to Feishu",
