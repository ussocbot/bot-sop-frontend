...
async function fetchBotInfo({ apiOrigin, tenantToken }) {
  const body = await fetchJson(`${apiOrigin}/open-apis/bot/v3/info`, {
    headers: { Authorization: `Bearer ${tenantToken}` }
  });
  return body.bot || body.data?.bot || null;
}

async function createTableRecord({ apiOrigin, tenantToken, appToken, tableId, fields }) {
  const body = await fetchJson(
    `${apiOrigin}/open-apis/bitable/v1/apps/${encodeURIComponent(appToken)}/tables/${encodeURIComponent(tableId)}/records`,
    {
      method: "POST",
      headers: {
        Authorization: `Bearer ${tenantToken}`,
  ...
  return body.data?.record || body.data || null;
}

async function deleteTableRecord({ apiOrigin, tenantToken, appToken, tableId, recordId }) {
  return fetchJson(
    `${apiOrigin}/open-apis/bitable/v1/apps/${encodeURIComponent(appToken)}/tables/${encodeURIComponent(tableId)}/records/${encodeURIComponent(recordId)}`,
    {
      method: "DELETE",
      headers: { Authorization: `Bearer ${tenantToken}` }
    }
...
module.exports = { fetchJson, getTenantToken, fetchTableRecords, fetchBotInfo, createTableRecord, deleteTableRecord };
